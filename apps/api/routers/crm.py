...
async def create_contact(payload: ContactCreate, db: DbDep) -> Contact:
    contact = Contact(
        org_id=_default_org_id(),
        name=payload.name,
        phone=payload.phone,
...
async def get_contact(contact_id: UUID, db: DbDep) -> Contact:
    stmt = (
        select(Contact)
...
async def update_contact(contact_id: UUID, payload: ContactUpdateIn, db: DbDep) -> Contact:
