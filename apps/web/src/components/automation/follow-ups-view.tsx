...
export interface FollowUpsViewProps {
  /** Locks this page to one channel — the Voice/WhatsApp Follow-ups pages
   * each render this with their own value, replacing the old in-page
   * channel column/picker. Activity (follow-up tasks) stays shared across
   * both, same as before, since tasks aren't gated per-channel. */
  channel: "voice" | "whatsapp";
}

export function FollowUpsView({ channel }: FollowUpsViewProps) {
  const rules = useFollowUpRules(channel);
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        title={channel === "voice" ? "Voice Follow-ups" : "WhatsApp Follow-ups"}
        description={
          channel === "voice"
            ? "Rules that automatically re-engage leads with an automated outbound call, plus every lead's own scheduled follow-up."
            : "Rules that automatically re-engage leads over WhatsApp, plus every lead's own scheduled follow-up."
        }
        action={<NewFollowUpRuleDialog channel={channel} />}
      />
