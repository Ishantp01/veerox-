...
import { ArrowRight, Megaphone, Phone, UserCheck } from "lucide-react";
...
/**
 * Right-rail panel for the Dial page — mirrors WhatsAppSendAnalyticsPanel's
 * "Message Analytics" + "Recent Campaigns" layout for the voice channel.
 */
export function CallAnalyticsPanel() {
  const stats = useStats();
  const campaigns = useCampaigns("voice");
        ...
        <CardContent className="grid grid-cols-2 gap-3">
          {stats.isLoading ? (
            ...
            <>
              <StatCard
                label="Calls today"
                value={stats.data?.calls_today ?? 0}
                icon={Phone}
                tint="emerald"
              />
              <StatCard
                label="Leads today"
                value={stats.data?.leads_today_voice ?? 0}
                icon={UserCheck}
                tint="purple"
              />
            </>
